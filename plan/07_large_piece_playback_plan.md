...
四阶段全部落地于分支 `copilot-fixes`（本地 commit，**未推送**；工作区 4 个无关已暂存
发布文件 ci.yml/release.yml/.gitignore/onefile.spec 全程未卷入）：

| 阶段 | commit | 内容 |
|---|---|---|
| 1 | `6046d72` | 调度窗口按字节（16MB）换算 + 窗口内 deadline 递增保序 + 尾窗排后 |
| 2 | `3b746e4` | 尾窗按 `min(size, max(2MB, 0.25%·size), 16MB)` 收敛 + 门控改判「尾部入口」+ `preview.demand` deadline 递增 |
| 2.5 | `75b834d` | 尾部入口块 deadline 提前到头窗头几块并行（门控再压） |
| 3 | `7383182` | 缓存占用口径改「已下载字节」+ 门控文案区分「等数据/等索引块」 |
| 4 | 本次文档 commit | 本完成记录 + README「大块种子的播放行为（已知取舍）」+ 删除一次性探针 |

### 关键实测数字（本机做种闭环，真链路）

**头块就绪（300MB / 4MB 块 / 做种端限 1MB/s，`probe_ab_diag.py` A/B）：**

| 组 | 头连续 ≥1 块用时 | 备注 |
|---|---|---|
| 改前（旧 `begin`） | **14.0s** | t=13s 仍 0.00MB，进度 11.9MB |
| 改后（新 `begin` = B 策略） | **5.0s** | 验收线 ≤10s；与手工 B 覆盖同量级（运行间波动） |
| 改后手工 B 覆盖 | 9.0s | t=13s 已连续 8.00MB |

→ 头块就绪 **14.0s → 5.0s（-64%）**。
该 300MB 几何旧/新**尾窗都恰 1 块**，故门控项对尾窗改动不敏感（头连续≥1块 9.0s、
门控成立时刻均「未出现（>17s）」）——这是几何特性，非回归。

**开播门控成立时刻（4.1GB / 4MB 块 / 做种端 2MB/s，稀疏载荷复现真机几何）：**

| 版本 | 尾窗 | 头连续≥1块 | 尾部入口就绪 | **门控成立** |
|---|---|---|---|---|
| 改前 | 11 块 44MB（整窗就绪才开） | 8.0s | 36.0s | **36.0s** |
| 阶段 2 后 | 3 块 ≈10MB，入口 1 块 | 8.0s | 19.0s | **19.0s（-47%）** |
| 阶段 2.5 后 | 同上，入口 deadline 提前到头窗并行 | 8.0s | 10.0s | **10.0s（= 头块 + 2.0s）** |

→ 门控 **36.0s → 19.0s → 10.0s**；头部行为全程逐字不变。
（各次运行的「头连续」在 7–9s 间波动，属运行间噪声，不影响结论方向。）

### 遗留项

- **一次性探针已删除**：收口时删除 `probe_ab_diag.py`。其关键逻辑/阈值已全部固化为
  常驻断言，无遗漏 —— `window_pieces` 16MB 换算表（`playback_window_test §A` +
  `contract_check` 换算表冻结）、窗口内 deadline 递增序（§B）、尾部入口就绪与排序契约
  （§C/§H）、`tail_window_bytes` 收敛表（§F/§G，含旧式 44MB/11 块对照）、
  `tail_entry_ready` / `tail_piece_window` / `tail_entry_pieces` 签名与常量冻结
  （`contract_check`）。计时指标（14.0s→5.0s、36→19→10s）已进本文件与
  `.workbuddy/memory/2026-09-10.md`；真机 4.1GB 组用的临时脚本
  （`%LOCALAPPDATA%\Temp\mv_probe41.py`）亦为一次性、未入库。
- **门控结构上无法再压到「与头块同时」**：门控必需的尾部入口块排在**播放窗口之后**
  （阶段 2.5 仅把它提前到头窗头几块并行，未提前整尾窗）。「头块就绪即开播」需取消尾部
  索引先落地的前提，会破坏顺序前缀保序（`playback_window_test §B/§C`）。当前
  10.0s = 头块 + 2.0s，已是「只提前入口、不提前整尾窗」约定下的下限。
- **客户端限速 + 大块硬停滞**（见「另记」）：libtorrent 层面交互问题，未修，已写进
  README「大块种子的播放行为（已知取舍）」的已知问题。
